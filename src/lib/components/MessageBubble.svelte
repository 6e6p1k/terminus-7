<script lang="ts">
	...
	const { colors, stagger: stg, isMobile } = store;

	const rot = $derived(stg ? ((i % 2 === 0 ? -1 : 1) * (0.5 + (i % 3) * 0.35)) : 0);
	const push = $derived(stg ? (i % 3) * 22 : 0);
	const accent = $derived(isAI ? colors.cyan : colors.mag);
...
<div
	...
	style="max-width: {isMobile ? '92%' : '68%'};
		align-self: {isAI ? 'flex-start' : 'flex-end'};
		margin-top: {i === 0 ? '10px' : (stg ? '22px' : '16px')};
...
>
	<div class="flex gap-2.5 items-start {isAI ? '' : 'flex-row-reverse'}">
		...
		<div
			...
			style="width: 44px; height: 44px; background: #0b0b14; border: 2px solid {accent}; font-family: 'VT323', monospace; color: {accent}; box-shadow: 0 0 12px {isAI ? `rgba(${colors.cyanGlow},0.5)` : `rgba(${colors.magGlow},0.5)`}; clip-path: polygon(0 0,100% 0,100% 80%,80% 100%,0 100%);"
		>
			{isAI ? 'SYS' : 'USR'}
		</div>
		...
		<div class="flex-1 min-w-0">
			...
			<div
				...
				style="background: #0b0b14;
					...
					color: {colors.text};
					box-shadow: 4px 4px 0 rgba(0,0,0,0.6), inset 0 0 18px {isAI ? `rgba(${colors.cyanGlow},0.08)` : `rgba(${colors.magGlow},0.08)`};"
			>
				{msg.content}
			</div>
			...
			{#if isAI}
				<div class="flex gap-1.5 mt-2">
					<button
						...
						style="background: #07070c; color: {colors.cyan}; border: 1px solid rgba({colors.cyanGlow},0.5); font-family: 'Share Tech Mono', monospace;"
						onmouseenter={(e) => { e.currentTarget.style.background = colors.cyan; e.currentTarget.style.color = '#07070c'; }}
						onmouseleave={(e) => { e.currentTarget.style.background = '#07070c'; e.currentTarget.style.color = colors.cyan; }}
					>
						▤ CPY
					</button>
					<button
					...
					<button
						...
						class="cursor-pointer text-[10px] py-1 px-1.5 tracking-[1px]"
						style="background: {store.feedback[msg.id] ? colors.mag : '#07070c'}; color: {store.feedback[msg.id] ? '#07070c' : colors.mag}; border: 1px solid {colors.mag}; font-family: 'Share Tech Mono', monospace;"
					>
